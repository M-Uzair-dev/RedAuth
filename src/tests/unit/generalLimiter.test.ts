...
const mockReq = (ip?: string) =>
  ({ ip, headers: {}, socket: { remoteAddress: ip } }) as any;
...
describe("generalLimiter", () => {
  it("should short-circuit and call next() when NODE_ENV is test", async () => {
    ...
    await middleware(mockReq("1.2.3.4"), mockRes() as any, next);
  ...
  it("should call next() for a valid request under the limit", async () => {
    ...
    (redis.zadd as Mock).mockResolvedValue(1);
    (redis.zremrangebyscore as Mock).mockResolvedValue(0);
    (redis.zcard as Mock).mockResolvedValue(1);
  ...
  it("should return 401 and not call next() when req.ip is missing", async () => {
    ...
    await middleware(mockReq(undefined), res as any, next);
  ...
  it("should fail open and call next() when Redis throws a non-appError", async () => {
    vi.stubEnv("NODE_ENV", "production");
    (redis.zadd as Mock).mockRejectedValue(new Error("ECONNREFUSED"));

    const next = vi.fn();
    const res = mockRes();
    const middleware = generalLimiter({ windowSeconds: 60, maxRequests: 5 });

    await middleware(mockReq("1.2.3.4"), res as any, next);
  ...
  it("should return 429 and not call next() when the rate limit is exceeded", async () => {
    ...
    (redis.zadd as Mock).mockResolvedValue(1);
    (redis.zremrangebyscore as Mock).mockResolvedValue(0);
    (redis.zcard as Mock).mockResolvedValue(6); // exceeds maxRequests: 5
    (redis.expire as Mock).mockResolvedValue(1);
