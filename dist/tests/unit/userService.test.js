...
describe("getUser", () => {
    ...
    it("should throw appError when user is not found and throwErrorIfNotFound is true", async () => {
        prisma.user.findUnique.mockResolvedValue(null);
        await expect(userService.getUser("non-existent-id", true)).rejects.toMatchObject({
            statusCode: 404,
...
describe("editUser", () => {
    ...
    it("should throw appError on P2025 error when throwErrorIfNotFound is true", async () => {
        prisma.user.update.mockRejectedValue(p2025);
        await expect(userService.editUser("non-existent-id", { name: "New Name" }, true)).rejects.toMatchObject({
            statusCode: 404,
            type: errorType.USER_NOT_FOUND,
        });
    });
    it("should rethrow unexpected errors", async () => {
        ...
        await expect(userService.editUser("any-id", { name: "New Name" })).rejects.toThrow("DB connection lost");
    });
});
describe("deleteUser", () => {
    it("should return false when user is not found (P2025)", async () => {
        prisma.user.delete.mockRejectedValue(p2025);
    ...
    it("should rethrow unexpected errors", async () => {
        ...
        await expect(userService.deleteUser("any-id")).rejects.toThrow("DB connection lost");
    });
});
