...
import { waitForEmail, getEmailBody, clearEmails, } from "@/tests/helpers/testMailpit.js";
let app;
beforeAll(async () => {
...
describe("POST /auth/resendVerificationEmail", () => {
    ...
    it("should return 200 silently when the email is already verified", async () => {
        ...
        await request(app).post("/auth/signup").send(data);
        // verify the email
        const emailMessage = await waitForEmail(data.email, "Verify Your Email Address");
        ...
        await expect(waitForEmail(data.email, "Verify Your Email Address")).rejects.toThrow();
    });
...
//# sourceMappingURL=resendVerificationEmail.test.js.map